...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      let tu = ["/login", "/api/auth/login", "/api/auth/logout"];
      function tc(e10) {
        let { pathname: t2 } = e10.nextUrl, r2 = tu.some((e11) => t2.startsWith(e11)), n2 = t2.startsWith("/_next") || t2.startsWith("/favicon") || t2.includes(".");
        if (r2 || n2) return Z.next();
        let i2 = e10.cookies.get("auth_token");
        if (!i2 || "authenticated" !== i2.value) {
          let t3 = new URL("/login", e10.url);
          return Z.redirect(t3);
        }
        return Z.next();
      }
      e.s(["config", 0, { matcher: ["/((?!_next/static|_next/image|favicon.ico).*)"] }, "middleware", () => tc], 96592);
      var td = e.i(96592);
